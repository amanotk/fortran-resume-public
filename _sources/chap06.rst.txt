...
ここではこれまではオマジナイとして使ってきた ``write(*,*)`` や ``read(*,*)`` の意味を理解し，ファイル入出力や，出力時の書式指定の仕方，さらには文字列処理の方法について学ぼう．
    ...
    - :doc:`sample1.f90 <chap06_sample1>` : 書式指定
    - :doc:`sample2.f90 <chap06_sample2>` : ファイル入出力(openとclose)
    - :doc:`sample3.f90 <chap06_sample3>` : ファイル入出力(アスキー形式)
    - :doc:`sample4.f90 <chap06_sample4>` : ファイル入出力(バイナリ形式)
    - :doc:`sample5.f90 <chap06_sample5>` : 文字列処理
    - :doc:`sample6.f90 <chap06_sample6>` : ファイル各行の逐次処理
