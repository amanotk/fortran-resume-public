...
ここではプログラムの動作を制御するための文法について学ぼう．と言っても覚えなければいけないことは ``if`` による条件分岐，``do`` による繰り返し，``select`` による条件分岐のみである．``goto`` という構文も存在するのだが，これはバグのもとになることから一般的には使わないほうが良いとされており，従ってここでも敢えて扱わない．
    ...
    - :doc:`sample1.f90 <chap04_sample1>` : 条件分岐(if)
    - :doc:`sample2.f90 <chap04_sample2>` : 反復処理(doループ1)
    - :doc:`sample3.f90 <chap04_sample3>` : 反復処理(doループ2)
    - :doc:`sample4.f90 <chap04_sample4>` : 反復処理(doループ3)
    - :doc:`sample5.f90 <chap04_sample5>` : 条件分岐(select)
